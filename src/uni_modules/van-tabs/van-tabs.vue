	...
	<view :class="tabsClass">
		<van-sticky :disabled="!sticky" :z-index="zIndex" :offset-top="offsetTop" :container="container"
			...
			<view :class="tabsWrapClass">
				...
				<scroll-view :scrollWithAnimation="scrollWithAnimation" :scroll-x="scrollable" :scroll-left="scrollLeft"
					:class="tabsScrollClass" :style="[color ? { 'border-color': color } : {}]">
					<view :class="tabsNavClass" :style="tabCardTypeBorderStyle">
						<view v-if="type === 'line'" class="van-tabs__line" :style="lineStyle" />
						<view v-for="(item, index) in tabs" :key="index" :class="item.tabsItemClass"
							:style="item.tabsItemStyle" @click="onTap(index)">
		...
		...VantComponent({
          ...
          { animated: this.animated }
            ...
            this.scrollable
					...
					//currentIndex 可能为null
					this.setCurrentIndex(this.currentIndex?this.currentIndex:0)
				...
				resize() {
					...
					Promise.all([
						...
						console.log(rect.width, lineRect.width)
						lineOffsetLeft +=
				...
				scrollIntoView() {
					...
					this.$nextTick(() => {
						...
						doGetRect = () => Promise.all([
							...(children.map((item, index) => this.getRect(
								`.van-tab-${this._uid}-${index}`, true))),
							this.getRect(`.van-tabs__nav-${this._uid}`)
						]).then((rects = []) => {
							const tabRects = rects.slice(0, -1).reduce((prev, curr) => prev.concat(
								curr), [])
